...
def plot_average(modelobj):
    ...
    ticks_str = []
    for t in ticks:
        ticks_str.append[modelobj.get_time_str(t)]
...
def plot_campus(modelobj, use_obj=None):
    ...
    ln, wd = 
			int(garage_size / modelobj.south_garage.garage_width), 
    ...
    data_r = 
			N.ones((modelobj.campus_way_road.length, 
    ...
    if use_obj == None:
        ...
        imgs.append( axs[2].text( .005, .5, 
					"Time: " + str(modelobj.get_time_str(modelobj.step))))
        ax.axis('off')
    else:
        ax = axs[2]
        imgs[2].remove()
