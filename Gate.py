...
class Gate(object):
    """ Creates a Gate object, that is used to generate vehicles. It is the
        start and end for the agents, travelling to the Road object and Garage
        object.
    """
    ...
    def __str__(self):
        ...
        out = "Gate: Dump *******************************" + "\n"
        out += "Gate: Total Number Of Agents Entered: " + str(
            len(self.agents_list)) + "\n"
        out += "Gate: Total Number Of Vehicles Entered: " + str(
            len(self.vehicle_list)) + "\n"
        out += "Gate: Total Number Of Agents Left: " + str(
            self.total_agents_left) + "\n"
        out += "Gate: Total Number Of Vehicles Left: " + str(
            self.total_vehicles_left) + "\n"
        out += "Gate: Number in going in Queue == " + str(
            self.q_going_in.qsize()) + "\n"
        out += "Gate: Number in going out Queue == " + str(
            self.q_going_out.qsize()) + "\n"
        out += "******************************************"
        return out

    def __init__(self):
        """ The default constructor for a Gate Object. Initializing both
            directional queues, master lists of agents and
            vehicles, and dictionaries
        """
    ...
    def estimate_vehicles(self, no_agent):
        """ A method which generates an estimate for the amounnt of vehicles
            that will arrive given an amount of agents that are enrolled in a
            class at any time
        """
        if (no_agent == 0):
            return 0
    ...
    def leave_gate(self):
        ''' A method which returns the vehicle if ant exist, if not it returns
            None
        '''
    ...
    def enter_gate(self, vehicle):
        ...
        if vehicle is not None:
            ...
            print("Gate: enter_gate(): Error, vehicle is None.",
                  file=sys.stderr)
    ...
    def avg_time_to_leave(self, time):
        ''' A method which returns the average time it took
            (In terms of timesteps) for the students to leave the gate after
            exiting school at the given time.
        '''
