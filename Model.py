...
class Model(object):
    ...
    def __init__(self, dt=0.25, trafficWeight=1, garage_diff = 0, sigma = 5):

        self.trafficWeight = trafficWeight

        self.gate = Gate.Gate(sigma)

        self.campus_way_road = Road.Road(2, 2, 3)

        # self.northGarage = Garage.Garage("North Garage", 300, 20, 20, 0, 1)
        self.south_garage = Garage.Garage("South Garage", num_spot=771 - garage_diff,
                                         num_carpool_spot=23,
